...
A successful tag push then starts [Post Facebook release](.github/workflows/facebook-release.yml). That file has to be on the default branch. It posts once to the [Rewrite Better Facebook Page](https://www.facebook.com/people/Rewrite-Better/61594555839619/). The Vietnamese title ends with "(English below)". A line of dashes separates that part from the English version. Each part includes the update guide at `https://rewrite-better-ai.vercel.app/update.html`. The link preview is the GitHub release page. If that version is already on the Page, the workflow skips. Running the desktop workflow by hand does not post, because that run is not a version tag.
